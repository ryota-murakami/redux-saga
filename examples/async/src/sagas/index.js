...
export function* nextRedditChange() {
  while (true) {
    ...
    console.log(`yield select(selectedRedditSelector) :` + prevReddit)
    yield take(actions.SELECT_REDDIT)
    console.log(`yield take(actions.SELECT_REDDIT)`)
    const newReddit = yield select(selectedRedditSelector)
    console.log(`yield select(selectedRedditSelector) :` + newReddit)
    const postsByReddit = yield select(postsByRedditSelector)
    console.log(`yield select(postsByRedditSelector): ` + postsByReddit)
    if (prevReddit !== newReddit && !postsByReddit[newReddit]) {
      yield fork(fetchPosts, newReddit)
      console.log(`yield fork(fetchPosts, newReddit)`)
    }
  }
}
